...
def analyze_complexity():
    try:
        ...
        if language != 'python':
            return jsonify({'error': 'Complexity analysis currently supports Python only'}), 400
...
def estimate_complexity(code):
    """Estimate time and space complexity using AST analysis"""
    try:
        tree = ast.parse(code)
        complexities = {
            'time': 'O(1)',
            ...
            if isinstance(node, ast.FunctionDef):
                for call in ast.walk(node):
                    if isinstance(call, ast.Call) and getattr(call.func, 'id', None) == node.name:
                        complexities['time'] = 'O(2ⁿ)'  # Assume worst case
...
def execute_program(language, file_info, input_data=''):
    try:
        ...
        exec_time = round((time.time() - start_time) * 1000, 2)  # ms
        
        return {
            'status': 'success' if result.returncode == 0 else 'error',
            'output': result.stdout.decode('utf-8') or result.stderr.decode('utf-8'),
